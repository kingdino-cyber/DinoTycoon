...
function handleAttack(attacker, target, room) {
  ...
  if (target.hp <= 0) {
    ...
    setTimeout(() => {
      if (!room.players[target.id] && !room.bots[target.id]) return;
      ...
      emitToRoom(room, 'playerRespawned', { id: target.id, x: target.x, y: target.y, hp: target.hp, maxHp: target.maxHp });
    }, respawnDelay);
  }
}
